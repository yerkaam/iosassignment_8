...
//  assignment_8
//
//  Created by Yerdaulet Orynbay on 23.11.2024.
//

import Foundation
import UIKit
import SnapKit
final class SportsController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.sportsNews.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "newCell", for: indexPath) as! NewCell
        
        let newsItem = viewModel.sportsNews[indexPath.row]
...
private extension SportsController {
    func layoutUI() {
        title = "Sports"
    ...
    func bindViewModel() {
        viewModel.didLoadSportsNews = { [weak self] news in
            self?.tableView.reloadData()
        }
    }
}
extension SportsController{
