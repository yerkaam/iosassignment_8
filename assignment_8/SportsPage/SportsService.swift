...
protocol SportsService {
    func getSportsNews(success: @escaping ([New]) -> Void, failure: @escaping (Error) -> Void)
    func getTopHeadLinesWithURLSession(success: @escaping ([New]) -> Void, failure: @escaping (Error) -> Void)
}


class SportsServiceImpl: SportsService {
    func getSportsNews(success: @escaping ([New]) -> Void, failure: @escaping (Error) -> Void) {
        ...
        let queryParams: Parameters = [
            ...
            "category": "sports" // Указываем категорию "sports"
        ]
