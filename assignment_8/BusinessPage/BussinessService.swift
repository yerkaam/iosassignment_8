...
import Foundation
import Alamofire
protocol BussinessService {
    func getBussinessNews(success: @escaping ([New]) -> Void, failure: @escaping (Error) -> Void)
    func getTopHeadLinesWithURLSession(success: @escaping ([New]) -> Void, failure: @escaping (Error) -> Void)
}


class BussinessServiceImpl: BussinessService {
    func getBussinessNews(success: @escaping ([New]) -> Void, failure: @escaping (Error) -> Void) {
        ...
        let queryParams: Parameters = [
            ...
            "category": "business" // Указываем категорию "sports"
        ]
